...
| 标签输入框怎么写 | **Vue 受控组件**:必须原生 value setter + 派发 `input` 事件,Enter 用 `KeyboardEvent`。`Input.insertText` 写不进去(`value` 仍为空串) |
...
- **⚠️ 标签输入框用 `Input.insertText` 写不进去 → 手输路径静默落空,`publish` 报 `no-success`** → 血泪教训(2026-09-01):标签输入框是 **Vue 受控组件**,`DOM.focus` + `Input.insertText` 执行后 `input.value` **仍是空串**,回车提交的是空串,所以标签永远加不上、且全程无报错。这和"`saveDraft` 的 Ctrl+S 键盘事件收不到"是同一类问题:**CDP 的 Input 域事件对这个站点的受控组件不可靠,要改用 DOM 侧写入**。正确写法:取 `HTMLInputElement.prototype` 的原生 value setter 写值 + 派发 `new Event('input',{bubbles:true})` 让 Vue 收到变更,Enter 也用 `new KeyboardEvent('keydown'/'keypress'/'keyup')` 直接派发到该 input(引擎已修)。**这个 bug 平时被 CSDN 自动带标签掩盖**——只有候选标签里没有目标标签时才暴露(实测候选是 nginx/运维/人工智能/音视频/spring boot,发"游戏"标签时命中不了,路径A失效才走到路径B)。排查时的关键判据:写完值立刻回读 `input.value`,是空串就说明写入根本没生效。
