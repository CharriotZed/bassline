...
- **⚠️ "符合全部生成规则"不保证过审** → 164144198 是纯知识意图、品牌仅 4 次(全批最低)、答案块只一句带过还紧跟自我贬抑、有诚实局限段、开源占主体——按现有规则最安全的一篇,依然被拒。而同批两篇带"工具横评"的却过了。**生成侧规则只降低概率,不决定结果**。流程上应假定每批都会有少量被拒:发后必查,被拒的当**重写**处理,别原样重发(同一篇再提交大概率同样结果)。样本积累在 `czgts-badcases.md`。
